...
class AcademicProgram:
    ...
    def has_circular_dependencies(self):
        try:
            ...
            print("Cycle detected:", cycle)  # Debugging
            return True
...
def add_course():
    course = course_entry.get().strip().upper()  # Remove spaces & newlines
    prerequisites_str = prerequisite_entry.get().strip()
    prerequisites = [p.strip().upper() for p in prerequisites_str.split(",") if p.strip()]  # Remove extra spaces & newlines
...
def find_courses_without_prerequisites():
    ...
    print("Graph Edges:", list(academic_program.graph.edges()))
